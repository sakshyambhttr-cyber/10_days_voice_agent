...
export async function POST(req: Request) {
  try {
    ...
    if (action === 'cancel_schedule') {
      pyScript = `import sys, json; from dotenv import load_dotenv; load_dotenv('.env.local'); sys.path.append('src'); from schedule_model import cancel_schedule; res = cancel_schedule(user_id='${userId}'); print(json.dumps({'success': res, 'cancelled': res}))`;
    } else if (action === 'immediate') {
      pyScript = `import sys, json, asyncio; from dotenv import load_dotenv; load_dotenv('.env.local'); sys.path.append('src'); from outbound import trigger_outbound_practice; res = asyncio.run(trigger_outbound_practice(user_id='${userId}', phone_number='${phoneNumber}', name='${name}')); print(json.dumps(res))`;
    } else {
      pyScript = `import sys, json; from dotenv import load_dotenv; load_dotenv('.env.local'); sys.path.append('src'); from schedule_model import create_or_update_schedule; res = create_or_update_schedule(user_id='${userId}', phone_number='${phoneNumber}', practice_topic='${practiceTopic}', preferred_time='${scheduledTime}', timezone='${timezone}'); print(json.dumps({'success': True, 'schedule': res}))`;
    }
