...
(cd backend && uv run python src/telephony/outbound/agent.py dev ${LIVEKIT_URL:+--url "$LIVEKIT_URL"} ${LIVEKIT_API_KEY:+--api-key "$LIVEKIT_API_KEY"} ${LIVEKIT_API_SECRET:+--api-secret "$LIVEKIT_API_SECRET"}) &
