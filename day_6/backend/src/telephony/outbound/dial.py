...
async def dial(
    phone_number: str, room_name: str, user_id: str = "default_user", name: str = ""
) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Place an outbound BolBuddy practice call."
    )
    parser.add_argument(
        "--to",
        required=True,
    ...
    asyncio.run(
        dial(target_destination, room_name, user_id=args.user_id, name=args.name)
    )

    print(
        f"Dispatched {AGENT_NAME} to room '{room_name}' to call '{target_destination}' for learner '{args.user_id}'."
    )
