...
export function BolBuddySessionView() {
  ...
  const activeAgentFromAttr =
    (attributes?.active_agent as ActiveAgentType) || undefined;
  ...
  const [extractedFeedback, setExtractedFeedback] =
    ...
    for (const msg of messages) {
      if (!msg.from?.isLocal && msg.message) {
        ...
        if (spokenScoreMatch) {
          ...
          if (
            lowerMsg.includes("ideas were clear") ||
            lowerMsg.includes("clear ideas")
          ) {
