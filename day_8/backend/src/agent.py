...
class Assistant(Agent):
    ...
    async def fetch_next_exercise(
        ...
        try:
            ...
            elif hasattr(context, "session") and hasattr(context.session, "proc") and hasattr(context.session.proc, "userdata"):
            ...
            logger.warning(f"Failed to auto-mark call outcome in fetch_next_exercise: {err}")
    ...
    async def score_spoken_answer(
        ...
        try:
            call_id = context.proc.userdata.get("call_id") if hasattr(context, "proc") and hasattr(context.proc, "userdata") else None
            if call_id:
                mark_call_outcome(call_id=call_id, outcome="success", completed_activities_inc=1)
        except Exception as e:
            logger.warning(f"Failed to auto-mark call outcome in score_spoken_answer: {e}")
    ...
    async def mark_call_outcome(
        ...
        logger.info(f"TOOL CALL: mark_call_outcome (outcome='{outcome}', reason='{reason}')")
        try:
            call_id = context.proc.userdata.get("call_id") if hasattr(context, "proc") and hasattr(context.proc, "userdata") else None
            if call_id:
                inc = 1 if outcome == "success" else 0
                res = mark_call_outcome(call_id=call_id, outcome=outcome, failure_reason=reason or None, completed_activities_inc=inc)
