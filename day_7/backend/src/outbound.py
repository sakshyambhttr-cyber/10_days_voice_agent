...
async def trigger_outbound_practice(
    ...
    active_room = room_name or f"outbound-{uid}-{int(time.time())}"
    ...
    try:
        ...
        if not config["is_configured"]:
            ...
            raise TelephonyConfigurationError(
                f"Missing configuration environment variables: {missing_str}"
            )
        ...
        try:
            await lk.room.create_room(api.CreateRoomRequest(name=active_room))
            ...
            metadata = json.dumps(
                {
                    ...
                    "name": target_name,
                    "call_id": call_id,
                    "practice_topic": practice_topic or "",
                }
            )
